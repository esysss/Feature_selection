...
with open('data1.pickle', 'rb') as handle:
    data1 = pickle.load(handle)[0]

with open('data2.pickle', 'rb') as handle:
    data2 = pickle.load(handle)[0]

with open('data3.pickle', 'rb') as handle:
    data3 = pickle.load(handle)[0]
...
Data1 = [ds1[:,data1[:n]] for n in feat_nums]
Data2 = [ds2[:,data2[:n]] for n in feat_nums]
Data3 = [ds3[:,data3[:n]] for n in feat_nums]
...
for fold in range(10):
    ...
    for d in range(len(feat_nums)):
        ...
        svm_y1_p = svm_classifier1.predict(x_train1)
        svm_y2_p = svm_classifier2.predict(x_train2)
        svm_y3_p = svm_classifier3.predict(x_train3)

        LG_y1_p = LG_classifier1.predict(x_train1)
        LG_y2_p = LG_classifier2.predict(x_train2)
        LG_y3_p = LG_classifier3.predict(x_train3)

        print("SVM & data1")
        ACC, PPV, NPV, recall, TNR, f1, mcc, TP, FP, TN, FN = f.printer(y_train1,svm_y1_p)
        data1_svm[feat_nums[d]] = f.pd_updater(ACC, PPV, NPV, recall, TNR, f1, mcc, TP, FP, TN, FN,
                                               "Fold-{}".format(str(fold+1)),data1_svm[feat_nums[d]])

        print("SVM & data2")
        ACC, PPV, NPV, recall, TNR, f1, mcc, TP, FP, TN, FN = f.printer(y_train2, svm_y2_p)
        data2_svm[feat_nums[d]] = f.pd_updater(ACC, PPV, NPV, recall, TNR, f1, mcc, TP, FP, TN, FN,
                                               "Fold-{}".format(str(fold + 1)), data1_svm[feat_nums[d]])
        print("SVM & data3")
        ACC, PPV, NPV, recall, TNR, f1, mcc, TP, FP, TN, FN = f.printer(y_train3, svm_y3_p)
        data3_svm[feat_nums[d]] = f.pd_updater(ACC, PPV, NPV, recall, TNR, f1, mcc, TP, FP, TN, FN,
                                               "Fold-{}".format(str(fold + 1)), data1_svm[feat_nums[d]])

        print("LogisticRegression & data1")
        ACC, PPV, NPV, recall, TNR, f1, mcc, TP, FP, TN, FN = f.printer(y_train1, LG_y1_p)
